...
async def summarize_conversation_with_llm(conversation_text, existing_profile):
    ...
    prompt = f"""
你是一位专业的服装店顶尖销售顾问和客户分析师。
请仔细分析下面提供的客户对话记录，并根据对话内容，为客户生成或更新一个详细的个人穿搭画像。
你的输出必须是一个结构化的JSON对象。

请严格按照以下JSON格式进行输出，字段内容需要根据对话信息进行推断和填写。如果对话中没有提到相关信息，请将该字段值留空或设为 "未知"。
在更新画像时，请在已有画像的基础上进行补充和修正，而不是完全替换。

[画像标签结构]
{{
    "风格偏好": "例如: 舒适休闲, 精致女人味, 设计感小众, 文艺复古, 通勤优雅, 高街潮流, 甜美少女, 甜酷风等",
    "体重": "例如: 偏胖, 微胖, 中等, 偏瘦",
    "体型特征": "例如: O型(腰腹丰满), A型(臀腿丰满), X型(匀称沙漏), Y型(肩宽), H型(直筒)",
    "身高": "例如: 155及以下, 160, 165, 170, 170以上",
    "身材关注点": "例如: 显瘦, 显高, 遮腰腹, 遮臀腿, 修饰肩颈, 修饰腿型, 舒适度优先",
    "穿搭禁忌": "例如: 紧身, 露胳膊, 露腰, 露腿, 高领",
    "消费决策特征": "例如: 爽快型(看中即买), 折扣敏感型(等活动), 顾问依赖型(需推荐), 谨慎比较型(多次考量)",
    "偏爱材质": "例如: 真丝/醋酸, 棉麻, 羊毛羊绒, 牛仔, 针织, 功能性面料",
    "试穿尺码": "例如: 上装M, 下装L",
    "穿搭场景": "例如: 日常居家, 约会社交, 通勤职场, 商务正式, 旅行度假, 运动户外, 宴会/婚礼",
    "职业属性": "例如: 企业高管, 专业人士, 体制内职员, 全职太太, 私营老板, 时尚/创意从业者, 学生",
    "消费实力": "例如: 一般(1k-3k/单), 有实力(3k-8k/单)",
    "核心需求": "例如: 显年轻, 显贵气, 提升气场, 减龄学院感, 日常百搭",
    "其他备注": "记录无法归入以上分类的其他重要信息"
}}
